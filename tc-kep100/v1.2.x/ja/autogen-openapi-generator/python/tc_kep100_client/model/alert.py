...
class Alert(ModelNormal):
    ...
    attribute_map = {
        ...
        'succeed_measure': 'succeedMeasure',  # noqa: E501
    }
    ...
    def __init__(self, target, condition, *args, **kwargs):  # noqa: E501
        ...
        Keyword Args:
            ...
            ftp (AlertFtp): [optional]  # noqa: E501
            path (str): 送信先パス（Webhookモード：通知先エンドポイント、FTPモード：保存先パス）   省略時：'/notify'（Webhookモード）、'/'（FTPモード）   <br>※FTPモードで指定したパスが存在しない場合は新たに生成されます。   ※FTPモード時は最後の'/'は無視されます。 . [optional] if omitted the server will use the default value of "/notify"  # noqa: E501
            ...
            connect_status (str): 接続状態   直近のアラート通知時における通知先との接続状態   Never confirmed：アラート登録後、まだ1度も通知していない or 通知先デバイスがUSB   Success：接続成功   Failure：接続失敗。アラート設定や通知先のファイアウォールなどを確認してください。 . [optional]  # noqa: E501
            succeed_measure (bool): 計測成功/失敗   直近のアラート監視時における計測の成否   true：計測成功   false：計測失敗もしくはアラート登録後一度も監視を行っていない   . [optional]  # noqa: E501
        """
